...
consists of a fluid part\footnote{Here $\t T=\boldsymbol{v}\v w$ means the outer product $\boldsymbol{v}\otimes\boldsymbol{w}$,
...
\begin{align}
m\ddot{\v r} & =\frac{e}{c}\dot{\v r}\times\v B+e\v E.\label{eq:Lorentz}
\end{align}
or written in terms of a \emph{linear} ``Lorentz'' operator for
homogeneous $\v B$ field:
\begin{equation}
\hat{L}\v r\equiv\left(\frac{\d^{2}}{\d t^{2}}+\frac{e\v B}{mc}\frac{\d}{\d t}\times\right)\v r=\frac{e}{m}\v E\label{eq:LorentzOperator}
\end{equation}
...
is linear, so the overall solution can be described as a linear superposition
of two solutions.
...
\subsection*{Solution for gyration}
...
\subsection*{3.4 Motion in weakly inhomogeneous fields}
...
$E_{\parallel}=\v b\cdot\v E$ it is approximately conserved -- an
\textbf{adiabatic invariant}. 
\begin{align}
...
and $(\hat{\v{\rho}},\hat{\v n},\hat{\v b})$ form an orthonormal
basis, where the latter depend on $\phi$. Variables have suspicious
...
becomes inhomogeneous, they approximate the particle values such that
their gyro-averages match closely for weak inhomogenity within the
space a particle transverses within one gyration. Similarly, $\v R$
describes the center of the Larmor circle only in the homogeneous
limiting case, and remains close to it in a field with sufficiently
weak gradients. To sum up one could say that $(\phi,v_{\parallel},v_{\perp})$
are values we would get if we pretend that $\v B$ is evaluated in
the guiding-center position $\v R$ rather than $\v R+\v{\rho}$.
In contrast to the earlier treatment, all variables in $\v z=(\v R,\phi,v_{\parallel},v_{\perp})$
have a well-defined relation to the particle position $\v r$ and
velocity $\v v$ if Eqs.~(\ref{eq:rz}-\ref{eq:vz}) are solved implicitly
in $\v z$. 
...
we break at the first order, but which can also be performed to higher
order. We use
\begin{align}
\v A(\v r) & =\v A(\v R)+\v{\rho}\cdot\nabla\v A(\v R)+\mathcal{O}(\rho_{c}^{2}),\\
\Phi(\v r) & =\Phi(\v R)+\v{\rho}\cdot\nabla\Phi(\v R)+\mathcal{O}(\rho_{c}^{2}).
\end{align}
Here the small parameter compared to typical length scales is the
gyroradius
\begin{equation}
...
is applicable and leaves the perpendicular velocity $v_{\perp}$ being
a quantity of interest with order $\varepsilon^{0}=1$ and thus prevents
it from being swallowed by a perturbation expansion. 

We further use the chain rule for
\begin{align}
\dot{\v{\rho}} & =\frac{\d}{\d t}\v{\rho}(\v R,\phi,v_{\perp})=\dot{\v R}\cdot\nabla\v{\rho}+\dot{\phi}\frac{\partial\v{\rho}}{\partial\phi}+\dot{v}_{\perp}\frac{\partial\v{\rho}}{\partial v_{\perp}}\nonumber \\
 & =\dot{\v R}\cdot\nabla\v{\rho}+\dot{\phi}\rho_{c}\hat{\v n}+\frac{\dot{v}_{\perp}}{v_{\perp}}\v{\rho}=\dot{\phi}\rho_{c}\hat{\v n}+\varepsilon(\dot{\v R}\cdot\nabla\v{\rho}+\frac{\dot{v}_{\perp}}{v_{\perp}}\v{\rho}).
\end{align}
...
we look at the bracket up to order $\varepsilon^{0}$,
\begin{align}
\left(mv_{\parallel}\hat{\v b}(\v R)+mv_{\perp}\hat{\v n}(\v R,\phi)+\frac{e}{c}\v A(\v R+\v{\rho})\right)\cdot(\dot{\v R}+\dot{\v{\rho}}) & =m\left(v_{\parallel}\hat{\v b}+v_{\perp}\hat{\v n}+\varepsilon^{-1}\frac{e}{mc}\v A+\frac{e}{mc}\v{\rho}\cdot\nabla\v A\right)\cdot\dot{\v R}\nonumber \\
 & +m\left(\varepsilon^{-1}\frac{e}{mc}\v A+\frac{e}{mc}\v{\rho}\cdot\nabla\v A\right)\cdot(\dot{\phi}\rho_{c}\hat{\v n})+v_{\perp}\dot{\phi}\rho_{c}\nonumber \\
 & +\frac{e}{mc}\v A\cdot(\dot{\v R}\cdot\nabla\v{\rho}+\frac{\dot{v}_{\perp}}{v_{\perp}}\v{\rho})+\mathcal{O}(\varepsilon).
\end{align}
In the second line, the term with $\hat{\v b}\cdot\hat{\v n}$ vanished
and we took a scalar product $\hat{\v n}\cdot\hat{\v n}=1$. Now we
use the fact that we can add/subtract an arbitrary total time derivative
to the Lagrangian without changing its dynamics. For this we take
\begin{align}
\frac{\d}{\d t}(\v{\rho}\cdot\v A(\v R)) & =\dot{\v{\rho}}\cdot\v A+\v{\rho}\cdot\dot{\v A}\nonumber \\
 & =\dot{\v{\rho}}\cdot\v A+\v{\rho}\cdot(\dot{\v R}\cdot\nabla\v A).
\end{align}
This we can combine with the term $(\v{\rho}\cdot\nabla\v A)\cdot\dot{\v R}=\dot{\v R}\cdot(\v{\rho}\cdot\nabla\v A)$
via
\begin{align}
\dot{\v R}\cdot(\v{\rho}\cdot\nabla\v A)-\v{\rho}\cdot(\dot{\v R}\cdot\nabla\v A) & =\dot{R}_{k}\rho_{i}\partial_{i}A_{k}-\rho_{k}\dot{R}_{i}\partial_{i}A_{k}\nonumber \\
 & =\dot{R}_{l}\rho_{m}(\delta_{lk}\delta_{mi}-\delta_{li}\delta_{mk})\partial_{i}A_{k}\nonumber \\
 & =\dot{R}_{l}\rho_{m}\varepsilon_{lmk}\varepsilon_{ijk}\partial_{i}A_{k}\nonumber \\
 & =(\dot{\v R}\times\v{\rho})(\nabla\times\v A)=(\dot{\v R}\times\v{\rho})\v B.
\end{align}
Thus we write
\begin{equation}
\dot{\v R}\cdot(\v{\rho}\cdot\nabla\v A)=(\dot{\v R}\times\v{\rho})\v B+\v{\rho}\cdot(\dot{\v R}\cdot\nabla\v A)=(\dot{\v R}\times\v{\rho})\v B+\frac{\d}{\d t}(\v{\rho}\cdot\v A)-\dot{\v{\rho}}\cdot\v A.
\end{equation}
Inserting everything into $L$ means
\begin{align}
L-\frac{\d}{\d t}(\v{\rho}\cdot\v A)+\mathcal{O}(\varepsilon) & =m\left(v_{\parallel}\hat{\v b}+v_{\perp}\hat{\v n}+\varepsilon^{-1}\frac{e}{mc}\v A\right)\cdot\dot{\v R}\nonumber \\
 & +m\left(\varepsilon^{-1}\frac{e}{mc}\v A+\frac{e}{mc}\v{\rho}\cdot\nabla\v A\right)\cdot(\dot{\phi}\rho_{c}\hat{\v n})+v_{\perp}\dot{\phi}\rho_{c}\nonumber \\
 & +\frac{e}{mc}\v A\cdot(\dot{\v R}\cdot\nabla\v{\rho}+\frac{\dot{v}_{\perp}}{v_{\perp}}\v{\rho})\nonumber \\
 & +\frac{e}{c}(\dot{\v R}\times\v{\rho})\v B-\dot{\v{\rho}}\cdot\v A\nonumber \\
 & -\left(\frac{m(v_{\parallel}^{\,2}+v_{\perp}^{\,2})}{2}+e\Phi\right),
\end{align}
where $\v A,\v B,$ and $\Phi$ are now evaluated in the guiding-center
position $\v R$ instead of the particle position $\v r$. When taking
an gyro-average over $\phi$ via
\[
\left\langle f\right\rangle (\v R,v_{\parallel},v_{\perp})\equiv\int_{0}^{2\pi}f(\v R,\phi,v_{\parallel},v_{\perp})\d\phi
\]
 of this expression a lot of terms will cancel out. Namely, 
\begin{equation}
\left\langle \hat{\v n}\right\rangle =\left\langle \hat{\v{\rho}}\right\rangle =0,\quad\left\langle \dot{\v{\rho}}\cdot\v W(\v R)\right\rangle =\mathcal{O}(\varepsilon),\quad\left\langle \frac{\dot{v}_{\perp}}{v_{\perp}}\right\rangle =\mathcal{O}(\varepsilon).
\end{equation}

To leading order in $\varepsilon$ and after performing a gyro-average
the \textbf{guiding-center Lagrangian} follows as
\[
L(\v R,\dot{\v R},\cancel{\phi},\dot{\phi},v_{\parallel},v_{\perp})=\left(mv_{\parallel}\hat{\v b}(\v R)+\frac{e}{c}\v A(\v R)\right)\cdot\dot{\v R}-\frac{m^{2}cv_{\perp}^{\,2}}{2eB(\v R)}\dot{\phi}-\left(\frac{m(v_{\parallel}^{\,2}+v_{\perp}^{\,2})}{2}+e\Phi(\v R)\right).
\]
...
\begin{equation}
\frac{\partial L}{\partial\dot{\phi}}=\frac{m^{2}cv_{\perp}^{\,2}}{2eB(\v R)}\equiv J_{\perp}=\frac{mc}{e}\mu,
\end{equation}
...
\begin{equation}
L(\v R,\dot{\v R},\dot{\phi},v_{\parallel},\mu)=\left(mv_{\parallel}\hat{\v b}(\v R)+\frac{e}{c}\v A(\v R)\right)\cdot\dot{\v R}-\frac{mc}{e}\mu\dot{\phi}-\left(\frac{mv_{\parallel}^{\,2}}{2}+\mu B(\v R)+e\Phi(\v R)\right).
\end{equation}
...
as
\begin{align}
\dot{\v R} & =\frac{v_{\parallel}\v B^{\star}(\v R,v_{\parallel})+\v E^{\star}(\v R,\mu)\times\hat{\v b}(\v R)}{\hat{\v b}(\v R)\cdot\v B^{\star}(\v R,v_{\parallel})},\quad\dot{v}_{\parallel}=\frac{e}{m}\frac{\v E^{\star}(\v R,\mu)}{\hat{\v b}(\v R)\cdot\v B^{\star}(\v R,v_{\parallel})},\label{eq:rdot-1}\\
...
together with (\ref{eq:Klimontovich}), or finding the Liénard--Wiechert
...
\nabla\cdot(\v a\v b)=(\v a\cdot\nabla)\v b+(\nabla\cdot\v a)\v b
\end{equation}
with $\v a=\v v$ and $\v b=\frac{\v F_{\alpha}f_{\alpha}}{m_{\alpha}}$,
so
\begin{equation}
\v v\left(\nabla_{\boldsymbol{v}}\cdot\frac{\v F_{\alpha}f_{\alpha}}{m_{\alpha}}\right)=\nabla_{\boldsymbol{v}}\cdot\frac{\v v\v F_{\alpha}f_{\alpha}}{m_{\alpha}}-\underbrace{(\nabla_{\v v}\cdot\v v)}_{=1}\frac{\v F_{\alpha}f_{\alpha}}{m_{\alpha}}.
\end{equation}
...
$\alpha$. Only the contribution $\v R_{\alpha\alpha}$ from self-collisions
...
\subsection*{6.2 Linear plasms response}
...
\begin{equation}
\rho^{\mathrm{ind}}(\v r,t)=\rho^{\mathrm{ind}}(\v r,t_{0})-\int_{t_{0}}^{t}\nabla\cdot\boldsymbol{J}^{\mathrm{ind}}(\v r,t)\,\d t^{\prime}\equiv\hat{\rho}\boldsymbol{J}^{\mathrm{ind}}=\hat{\rho}\hat{\v{\sigma}}\v E.
\end{equation}
This is again a linear operator acting on $\v E$ by combining $\hat{\rho}$
and $\hat{\v{\sigma}}$. Inserting these relations into the first
two Maxwell equations yields
\begin{align}
...
\end{equation}
Taking a time derivative of Ampère/Maxwell and inserting Faraday for
